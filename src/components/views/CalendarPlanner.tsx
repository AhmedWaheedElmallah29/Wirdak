...
import { getForecastByDate } from "../../data/mockData";
...
const calcStreak = (activityByDate: Record<string, ActivityLog[]>): number => {
  ...
  while (true) {
    ...
    if (!activityByDate[key] || activityByDate[key].length === 0) break;
    streak++;
...
const DayPanel: React.FC<DayPanelProps> = ({
  ...
  const { t, lang, formatRangeInfo } = useI18n(); // 👈 سحبنا دالة formatRangeInfo
  if (!date) return null;
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <h3 className="font-bold text-slate-900 dark:text-slate-50 text-base leading-tight mt-0.5">
              {title}
            </h3>
          </div>
          <button
            ...
            <>
              {activity.length === 0 ? (
                ...
                activity.map((a) => (
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${a.action === "memorized" ? "bg-violet-500" : "bg-emerald-500"}`}
                    />
                    <div className="flex-1 min-w-0">
                      {lang === "ar" ? (
                        <p className="text-sm font-semibold font-arabic text-slate-800 dark:text-slate-100 truncate">
                          {a.surahArabicName}
                        </p>
                      ) : (
                        <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">
                          {a.surahName}{" "}
                          <span className="text-xs font-normal text-slate-500 font-arabic">
                            {a.surahArabicName}
                          </span>
                        </p>
                      )}
                    ...
                    >
                      {a.action === "memorized"
                        ? t.calendar.actionMemorized
                        : t.calendar.actionReviewed}
                    </span>
                  </div>
                ))
              )}
                ...
                forecast.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500">
                        {/* 👈 استخدمنا الدالة المجهزة عشان منضربش إيرور */}
                        {formatRangeInfo(c.rangeInfo)}
                      </p>
                    </div>
...
// ─── Main component ──────────────────────────────────────────────────────────
const CalendarPlanner: React.FC<CalendarPlannerProps> = ({ chunks }) => {
  const { t, lang, formatRangeInfo } = useI18n(); // 👈 سحبنا الدالة هنا كمان
  const { userId } = useAuth();
  const [realActivityByDate, setRealActivityByDate] = useState<
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        const formattedActivity: Record<string, ActivityLog[]> = {};

        // 👈 حلينا مشكلة الـ any بنوع الداتا ده
        type DBActivity = {
          _id: string;
          chunkId: string;
        ...
        dbProgress.forEach((p: DBActivity) => {
          if (p.lastReviewedDate) {
            const date = new Date(p.lastReviewedDate)
              .toISOString()
              .split("T")[0];
            if (!formattedActivity[date]) formattedActivity[date] = [];

            formattedActivity[date].push({
              id: p._id || p.chunkId,
              chunkId: p.chunkId,
              action: "reviewed",
              date: p.lastReviewedDate,
        ...
        setRealActivityByDate(formattedActivity);
      } catch (error) {
  ...
  const streak = useMemo(
    () => calcStreak(realActivityByDate),
    [realActivityByDate],
  );
  const totalActiveDays = useMemo(
    () => Object.keys(realActivityByDate).length,
    [realActivityByDate],
  );

  // 👈 شيلنا useCallback عشان نحل خطأ الـ Compilation
  const prevMonth = () => {
    if (month === 0) {
      setMonth(11);
      setYear(year - 1);
  ...
  const selectedActivity = selectedDate
    ? (realActivityByDate[selectedDate] ?? [])
    : [];
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="card p-5 flex flex-col gap-1 border-emerald-200 dark:border-emerald-800">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {t.calendar.streakDays(streak)}
          </p>
        </div>
        <div className="card p-5 flex flex-col gap-1">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">
            {t.calendar.totalDays}
          </p>
          <p className="text-3xl font-bold text-slate-900 dark:text-slate-50">
            {totalActiveDays}
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {t.calendar.streakDays(totalActiveDays)}
          </p>
        </div>
      </div>
      ...
      <div className="card p-4 sm:p-6">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: days }).map((_, i) => {
            ...
            const acts = realActivityByDate[dateStr] ?? [];
      ...
      <div className="card p-5">
        ...
        {(() => {
          const upcoming = Object.entries(forecastByDate)
            .filter(([d]) => {
              const limit = new Date(Date.now() + 14 * 86_400_000)
                .toISOString()
                .split("T")[0];
            ...
            >
              ...
              <div className="flex-1 flex flex-wrap gap-1.5">
                {fc.map((c) => (
                  <span
                    key={c.id}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 font-medium"
                  >
                    {lang === "ar" ? (
                    ...
                    {/* 👈 استخدمنا دالتنا النضيفة برضه هنا */}
                    {formatRangeInfo(c.rangeInfo)}
                  </span>
                ))}
              </div>
